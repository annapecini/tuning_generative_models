...
parent_path = Path(f'exp/{ds_name}/ctgan_best/')
...
def train_ctgan():
    ...
    model = CTGAN(field_transformers=field_transformers, **parameters)
    model.fit(data)
    model.save(os.path.join(parent_path, 'model.pkl'))


def sample_ctgan():
    loaded = CTGAN.load(os.path.join(parent_path, 'model.pkl'))
    ...
    new_data.to_csv(os.path.join(parent_path, 'synthetic_ctgan.csv'), index=False)
