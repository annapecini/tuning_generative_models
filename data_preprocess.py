...
def main():
    df = pd.read_csv('nike_sales_short.csv')
    ...
    df['GPD_per_capita'] = pd.to_numeric(df['GPD_per_capita'], downcast="float")
    df['eu_prod_index'] = pd.to_numeric(df['eu_prod_index'], downcast="float")
    ...
    X = df.drop(['value'], axis=1).values.tolist()
    y = df['value'].tolist()
    ...
    # Save npy files
    #X_num_train = np.array([np.hstack((a_1d[:1], a_1d[2:4], a_1d[5:6])) for a_1d in X_train])
    #X_cat_train = np.array([np.hstack((a_1d[1:2], a_1d[4:5], a_1d[6:9])) for a_1d in X_train])

    #X_num_test = np.array([np.hstack((a_1d[:1], a_1d[2:4], a_1d[5:6])) for a_1d in X_test])
    #X_cat_test = np.array([np.hstack((a_1d[1:2], a_1d[4:5], a_1d[6:9])) for a_1d in X_test])

    #X_num_val = np.array([np.hstack((a_1d[:1], a_1d[2:4], a_1d[5:6])) for a_1d in X_val])
    #X_cat_val = np.array([np.hstack((a_1d[1:2], a_1d[4:5], a_1d[6:9])) for a_1d in X_val])
